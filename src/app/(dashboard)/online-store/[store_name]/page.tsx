...
const OnlineStorePage: React.FC = () => {
  ...
  try {
    ...
    // Set productss to the products array or an empty array if not present
...
const handleImageUpload = (file: File, type: "banner" | "profile") => {
    try {
      if (type === "banner") {
        ...
        // Update preview with a temporary URL
  ...
  const handleSaveBannerChanges = async () => {
    try {
      ...
      // Close the editing overlay
      setEditingBanner(false);
      setSelectedBannerFile(null); // Clear the selected file
    } catch (error) {
      ...
      <section className="relative bg-gradient-to-r from-indigo-600 to-purple-600 text-white">
        ...
        {storeSettings.bannerImage && (
          <div className="absolute inset-0">
            <Image
              src={productss.banner_image || "/images/placeholder.svg"}
              alt=""
              fill
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                onClick={() => {
                  setEditingBanner(false);
                  setSelectedBannerFile(null); // Clear selected file on cancel
                }}
