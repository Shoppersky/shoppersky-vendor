...
// }) => {
//   const colorClasses = {
//     cyan: "bg-cyan-50 border-cyan-100 hover:shadow-cyan-200/50",
//     green: "bg-green-50 border-green-100 hover:shadow-green-200/50",
//     red: "bg-red-50 border-red-100 hover:shadow-red-200/50",
//     indigo: "bg-indigo-50 border-indigo-100 hover:shadow-indigo-200/50",
//   };

//   return (
//     <Card className={`transition-all hover:-translate-y-1 hover:shadow-lg ${colorClasses[color]} dark:bg-zinc-800/50`}>
...
//                 {trend === "up" ? <ArrowUpDown className="h-3 w-3" /> : <ArrowUpDown className="h-3 w-3 rotate-180" />}
//                 {trendValue}
//               </div>
//             )}
//           </div>
//           <div className="p-2 rounded-full bg-white/50 dark:bg-zinc-700/50">{icon}</div>
//         </div>
//       </CardContent>
//     </Card>
//   );
...
//     setLoading(true);
//     try {
//       const response = await axiosInstance.get<ProductResponse[]>(`/products/by-vendor-id/${userId}`);
//       const mappedProducts: Product[] = response.data.map((product) => ({
//         id: product.product_id,
//         name: product.identification.product_name,
//         price: product.pricing?.selling_price
//           ? `$${Number.parseFloat(product.pricing.selling_price).toFixed(2)}`
//           : "$0.00",
//         image: product.images?.urls?.[0] || "/placeholder.svg",
...
//         subcategory: product.subcategory_name,
//         createdDate: product.timestamp.split("T")[0],
//         stock: product.inventory?.quantity ? Number.parseInt(product.inventory.quantity) : 0,
//         slug: product.slug,
//         sku: product.identification.product_sku,
//         shortDescription: product.descriptions?.short_description || "",
//       }));
...
//     try {
//       await axiosInstance.put(`/products/slug/${productToDelete.slug}/delete`);
//       setProducts((prev) => prev.filter((product) => product.slug !== productToDelete.slug));
...
//     const csvContent = [
//       ["ID", "Name", "Price", "Category", "Stock", "Status", "Created Date"],
//       ...filteredProducts.map((product) => [
//         product.id,
//         product.name,
//         product.price,
...
//         },
//         images: {
//           urls: [formData.image || "/placeholder.svg"],
//         },
//         descriptions: {
//           short_description: formData.shortDescription,
//         },
//         status_flags: {
//           product_status: formData.status === "Inactive",
//         },
//         vendor_id: userId,
//         category_name: formData.category,
//       };
...
//           id: response.data.product_id,
//           name: response.data.identification.product_name,
//           price: `$${Number.parseFloat(response.data.pricing?.selling_price || "0").toFixed(2)}`,
//           image: response.data.images?.urls?.[0] || "/placeholder.svg",
//           images: response.data.images?.urls || [],
//           sold: 0,
//           status: response.data.status_flags.product_status ? "Inactive" : "Active",
//           category: response.data.category_name,
//           subcategory: response.data.subcategory_name,
//           createdDate: response.data.timestamp.split("T")[0],
//           stock: Number.parseInt(response.data.inventory?.quantity || "0"),
...
//       console.error(error);
//     }
//   }, [editingProduct, formData, userId]);
...
//         <Card className="max-w-2xl mx-auto mt-12 bg-red-50 dark:bg-red-900/20">
...
//                 ) : (
//                   filteredProducts.map((product) => (
//                     <TableRow key={product.id} className="hover:bg-gray-50 dark:hover:bg-zinc-700/50">
//                       <TableCell className="py-4">
//                         <div className="flex items-center gap-3">
//                           <img
//                             src={product.image}
...
//                       <TableCell>
//                         <div className="flex items-center gap-2">
//                           <span className={product.stock <= 5 && product.status === "Active" ? "text-red-600" : ""}>
//                             {product.stock}
//                           </span>
